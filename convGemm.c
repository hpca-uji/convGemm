                        ...
                        float *ac_pack, float *bc_pack)
{
    ...
    int wo = (w + 2 * hpadding - hdilation * (kw - 1) - 1) / hstride + 1;
    convol_dim dim = { b, h, w, c, kn, kh, kw, vstride, hstride, vpadding, hpadding, vdilation, hdilation, ho, wo };
    gemm_blis_B3A2C0('C', 'C', 'C', 'T', 'N', c * kh * kw, ho * wo * b, kn, 1.0, weights, kn, dy, kn, 1.0, dx, c * kh * kw, ac_pack, pack_RB, bc_pack, pack_CB, NULL, post_row2im_nhwc, cntx, &dim, NULL, NULL, NULL, NULL, NULL, false);
}
                    ...
                    float *ac_pack, float *bc_pack)
{
    ...
    convol_dim dim = { b, h, w, c, kn, kh, kw, vstride, hstride, vpadding, hpadding, vdilation, hdilation, ho, wo };
    if (trans == 'N') {
        gemm_blis_B3A2C0('C', 'C', 'C', 'N', 'N', ho * wo * b, kn, kh * kw * c, 1.0, x, ho * wo * b, in, kh * kw * c, 0.0, out, ho * wo * b, ac_pack, pack_RB_nchw, bc_pack, pack_CB, NULL, add_bias_transpose_nchw, cntx, &dim, bias_vector, bn_running_mean, bn_inv_std, bn_gamma, bn_beta, relu);
    } else {
        gemm_blis_B3A2C0('C', 'C', 'C', 'T', 'N', kh * kw * c, kn, ho * wo * b, 1.0, x, ho * wo * b, in, ho * wo * b, 0.0, out, kh * kw * c, ac_pack, pack_RB_nchw, bc_pack, pack_CB_nchw_trans, NULL, NULL, cntx, &dim, NULL, NULL, NULL, NULL, NULL, false);
    }
}
                        ...
                        float *ac_pack, float *bc_pack)
{
    ...
    int wo = (w + 2 * hpadding - hdilation * (kw - 1) - 1) / hstride + 1;
    convol_dim dim = { b, h, w, c, kn, kh, kw, vstride, hstride, vpadding, hpadding, vdilation, hdilation, ho, wo };
    gemm_blis_B3A2C0('C', 'C', 'C', 'N', 'T', b * ho * wo, c * kh * kw, kn, 1.0, dy, b * ho * wo, weights, c * kh * kw, 1.0, dx, b * ho * wo, ac_pack, pack_RB_nchw_trans, bc_pack, pack_CB, NULL, post_col2im_nchw, cntx, &dim, NULL, NULL, NULL, NULL, NULL, false);
}
