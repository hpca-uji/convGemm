                ...
                for (int jr = 0; jr < nc; jr += NR) {
                    for (int ir = 0; ir < mc; ir += MR) {
                        ...
                        if (postprocess == NULL) { // don't use buffer
                            gemm_kernel(mr, nr, kc, &alpha, &Ac[tid * MC * KC + ir * kc], &Bc[jr * kc], &betaI, Cptr, 1, ldC, &aux, cntx);
                        } else { // use buffer for postprocessing
                            gemm_kernel(mr, nr, kc, &alpha, &Ac[tid * MC * KC + ir * kc], &Bc[jr * kc], &zero, Clocal, 1, MR, &aux, cntx);
