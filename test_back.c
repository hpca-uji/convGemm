...
int main(int argc, char *argv[])
{
    ...
    gemm_blis_B3A2C0('C', 'C', 'C', 'T', 'N', c * kh * kw, ho * wo * b, kn, alpha, kernel, kn, out, kn, 1.0, image, c * kh * kw, ac_pack, pack_RB, bc_pack, pack_CB, post_row2im_nhwc, cntx, &dim);
    double t4 = get_time();
    double t_gemm = t2 - t1;
    ...
    gemm_blis_A3B2C0('C', 'C', 'C', 'T', 'N', c * kh * kw, ho * wo * b, kn, alpha, kernel, kn, out, kn, 1.0, image2, c * kh * kw, ac_pack, pack_RB, bc_pack, pack_CB, post_row2im_nhwc, cntx, &dim);
    t2 = get_time();
    t_nhwc = t2 - t1;
    ...
    t4 = get_time();
    memset(image, 0, b * h * w * c * sizeof(float));
    gemm_blis_B3A2C0('C', 'C', 'C', 'N', 'T', b * ho * wo, c * kh * kw, kn, alpha, out, b * ho * wo, kernel, c * kh * kw, 1.0, image, b * ho * wo, ac_pack, pack_RB_nchw_trans, bc_pack, pack_CB, post_col2im_nchw, cntx, &dim);
    double t5 = get_time();
    ...
    t1 = get_time();
    memset(image2, 0, b * h * w * c * sizeof(float));
    gemm_blis_A3B2C0('C', 'C', 'C', 'N', 'T', b * ho * wo, c * kh * kw, kn, alpha, out, b * ho * wo, kernel, c * kh * kw, 1.0, image2, b * ho * wo, ac_pack, pack_RB_nchw_trans, bc_pack, pack_CB, post_col2im_nchw, cntx, &dim);
    t2 = get_time();
    t_nchw = t2 - t1;
